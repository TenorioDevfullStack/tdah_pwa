...
export async function initMessaging() {
  ...
  let swReg = null;
  try {
    swReg = await navigator.serviceWorker.ready; // registro do /sw.js (next-pwa)
  } catch {}
  const vapidKey = process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY;
  const token = await messaging.getToken({ vapidKey, serviceWorkerRegistration: swReg }).catch(() => null);
